...
class OperationMap():
    ...
    types, or differences in capitalization for keys that retrieve them. For example,
    the name of a sample from an input labeled 'Polymerase' would be added the same
    list as the value of a DataAssociation with the key 'polymerase'. This list would
    be retrieved using the key 'polymerase'.
    ...
    def collect_samples(self, field_values: List[FieldValue]) -> Dict[str, list]:
